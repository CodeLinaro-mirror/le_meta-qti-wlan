...
PREFERRED_VERSION_soc-modules = "6.12"
...
EXTRA_OEMAKE += "SOC_REPO_INCDIR=${STAGING_INCDIR}/soc-repo/include"

EXTRA_OEMAKE += "'EXTRA_CFLAGS=-I${S}/inc -I${S}/cnss_utils -I${STAGING_INCDIR}/soc-repo/include -I${STAGING_INCDIR}/soc-repo/include/linux -I${STAGING_KERNEL_DIR}/include -I${STAGING_KERNEL_DIR}/include/linux'"
...
do_compile() {
    if [ -f ${STAGING_INCDIR}/soc-repo/include/linux/mhi.h ]; then
        cp --remove-destination \
           ${STAGING_INCDIR}/soc-repo/include/linux/mhi.h \
           ${STAGING_KERNEL_DIR}/include/linux/mhi.h
     fi
     oe_runmake CC="${KERNEL_CC}" \
         -C ${STAGING_KERNEL_BUILDDIR} \
         ${PLATFORM_MODULE_FLAGS} M=${S} \
         KBUILD_EXTRA_SYMBOLS="${STAGING_INCDIR}/kernel-module-soc-repo/Module.symvers" \
         modules
}
