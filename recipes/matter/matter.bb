...
do_configure() {
    #make sure python 3 version is at least 3.8
    export PATH=${PY3_PATH}:${PATH}
    cd ${S}
    source scripts/activate.sh

    cd ${S}/examples/all-clusters-app/linux
    gn gen out/test --args='is_debug=false treat_warnings_as_errors=false target_os="linux" target_cpu="arm64" custom_toolchain="${S}/build/toolchain/custom" target_cc="${CC}" target_cxx="${CXX}" target_ar="${AR}"'
