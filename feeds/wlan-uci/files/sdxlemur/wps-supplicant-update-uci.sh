	...
	if [ -r /var/run/wpa_supplicant-${ifname_overwrite} ]; then
		...
		case $auth_overwrite in
			...
			;;
				...
				uci set wireless.${sect}.sae='1'
				uci set wireless.${sect}.sae_password=''
			;;
...
config_get wps_pbc "$sect" wps_pbc 0
config_get wps_config "$sect" wps_config ''
[ "$wps_pbc" -gt 0  ] && append config_methods push_button

if [ "$wps_pbc" -le 0 ] && [ -z "$wps_config"]; then
	kill $(cat /var/run/wpa_cli-wps-${IFNAME}.pid)
fi
...
case "$CMD" in
	CONNECTED)
		...
		if [ -e "/var/run/wps-hotplug-$IFNAME.pid" ]; then
