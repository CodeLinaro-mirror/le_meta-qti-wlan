...
hostapd_set_bss_options() {
	...
	local tmp_vif="$2"
	local flag1=1
	local hidden beacon_prot versn_name
	local auth_algs
	...
	local macfilter macfile ifname
	local maxassoc
	local mld
	local num_ready_link

	[ -z "$vif" ] && hostapd_get_vif_name

	config_load wireless
	#Openwrt v19.07 needs differet handling while fetching vifs
	[ -f /etc/openwrt_release ] && versn_name=`cat /etc/openwrt_release | grep 'RELEASE'`
	case $versn_name in
		*19.07*)
			flag1=0
			for temp in $vif; do
	...
	config_get ifname "$vif" ifname # wlan0

	config_get mld "$vif" mld
	[ -n "$mld" ] && {
		# single link MLD required
		append "$var" "mld_ap=1" "$N"
		local mld_macaddr
		local bssid
		config_get mld_macaddr "$mld" mld_macaddr
		config_get bssid "$vif" bssid
		if [ -n "$mld_macaddr" ] ; then
			append "$var" "mld_addr=$mld_macaddr" "$N"
		fi
		if [ -n "$bssid" ] ; then
			append "$var" "bssid=$bssid" "$N"
		fi

		num_ready_link=$(uci get wireless.${mld}.num_ready_link)
		# TODO: any other mld parameters?
	}
	...
	[ -f ${wlan_module_path}/hostapd-$phy/$ifname ] && rm ${wlan_module_path}/hostapd-$phy/$ifname

	if [ "$map" -gt 0 ]
	then
		ctrl_interface=${wlan_module_path}/hostapd-$device
	else
		if [ -n "$mld" ] && [ "$num_ready_link" -gt 0 ]; then
			ctrl_interface=${wlan_module_path}/hostapd-$num_ready_link
		else
			ctrl_interface=${wlan_module_path}/hostapd
		fi
	fi
	...
	then
		...
		case "$enc" in
			...
			;;
			# Need ccmp*|gcmp* check for SAE and OWE auth type
			wpa2*|wpa3*|*psk2*|ccmp*|gcmp*|dpp|sae*)
				wpa=2
			;;
			*)
				wpa=3
	...
	# wps_pushbutton for standard uci option.
	# wps_pbc for ezmesh private uci option.
	config_get_bool wps_pbc "$vif" wps_pushbutton 0
	[ "$wps_pbc" -eq 0 ] && config_get_bool wps_pbc "$vif" wps_pbc 0
	config_get_bool wps_label "$vif" wps_label 0

	config_get wds_sta "$vif" wds_ext 0
	...
	[ "$wps_pbc" -gt 0 ] && append config_methods "keypad virtual_push_button"
	[ "$wps_label" -gt 0 ] && append config_methods "label virtual_display"
	...
	[ -n "$wps_possible" -a -n "$config_methods" ] && {
		...
		# Do not allow external registra to refresh AP settings
	...
	then
		...
		if [ "${ieee80211w}" -eq 2 ] || [ "${ieee80211w}" -gt 0 -a "$mld" ] || [ $enc == wpa3* ]
		then
			add_sha256=1
		fi
	...
	if [ "$multi_cred" -gt 0 ]; then
		...
		append "$var" "extra_cred=${wlan_module_path}/hostapd_cred_${device}.bin" "$N"
	fi
	...
	if [ -n "$mld" ]; then
		append "$var" "bss_transition=0" "$N"
	else
		if [ -n "$bss_transition" ]; then
			append "$var" "bss_transition=$bss_transition" "$N"
		else
	...
	# Multi-AP Specification v2.0 Table 3. Multi-AP IE format
	# Element ID | Length |    OUI     | OUI Type | Multi-AP Extension subelement
	#    0xDD    |  0x07  | 0x50-6F-9A |   0x1B   |  0x06-01-(Subelement Value)
	if [ $(($((MapBSSType&64)) >> 6)) -eq 1 ] && [ $(($((MapBSSType&32)) >> 5)) -eq 1 ]; then
		append "$var" "multi_ap=3" "$N"
		append "$var" "assocresp_elements=dd07506f9a1b060160" "$N"
	elif [ $(($((MapBSSType&32)) >> 5)) -eq 1 ]; then
		append "$var" "multi_ap=2" "$N"
		if [ $wps_pbc -eq 1 ]; then
			append "$var" "assocresp_elements=dd07506f9a1b060160" "$N"
		else
			append "$var" "assocresp_elements=dd07506f9a1b060120" "$N"
		fi
	elif [ $(($((MapBSSType&64)) >> 6)) -eq 1 ]; then
		config_get map_vlan "$vif" map8021qvlan 0
		append "$var" "multi_ap=1" "$N"
		if [ $map_vlan -gt 0 ]; then
			append "$var" "assocresp_elements=$(printf "dd0e506f9a1b0601400701020802%02x%02x" $((map_vlan&0xff)) $(($((map_vlan&0xf00)) >> 8)))" "$N"
		else
			append "$var" "assocresp_elements=dd07506f9a1b060140" "$N"
		fi
	fi
	...
	then
		append "$var" "wowlan_triggers=$wowlan_triggers" "$N"
	else
		append "$var" "wowlan_triggers=magic_pkt" "$N"
	fi

	config_get macfilter "$vif" macfilter
	if [ -n "$macfilter" ];then
		config_get ifname "$vif" ifname
		macfile="${wlan_module_path}/hostapd-$ifname.maclist"
		case "$macfilter" in
			allow)
		...
		[ -n "$macfile" ] && {
			...
			config_list_foreach "$vif" maclist append_mac_list
		}
	fi

	config_get hostapd_options "$device" hostapd_options
	[ -n "$hostapd_options" ] && append "$var" "$hostapd_options" "$N"
...
hostapd_config_multi_cred() {
	...
	cred_config="100e$temp$extra_cred"
		...
		cat > ${wlan_module_path}/hostapd_cred_tmp.conf <<EOF
$cred_config
EOF
		sed 's/\([0-9A-F]\{2\}\)/\\\\\\x\1/gI' ${wlan_module_path}/hostapd_cred_tmp.conf | xargs printf >> ${wlan_module_path}/hostapd_cred_$device.bin
}


hostapd_setup_vif() {
	qlog_cmd "enter hostapd_setup_vif vif=$1"
	local vif="$1" && shift
	...
	local is_hwmode_any=0
	local concurrency_mode
	local mld num_ready_link has_6GHz_link
	hostapd_cfg=
	...
	config_get_bool vht_11ng "$vif" vht_11ng 0
	config_get mld "$vif" mld

	if [ -n "$mld" ]; then
		num_ready_link=$(uci get wireless.${mld}.num_ready_link)
		has_6GHz_link=$(uci get wireless.${mld}.has_6GHz_link)
	fi

	qlog_cmd "before set band"
	...
	if { [ $band -eq 0 ] || [ $band -eq 2 ]; } && { \
		...
		qlog_cmd "check band and channel done"
	fi
	qlog_cmd "before hostapd_set_log_options"
	hostapd_set_log_options hostapd_cfg "$device"
	qlog_cmd "before config_load fst"
	config_load fst && {
	...
	if [ "$driver" == "nl80211" ]; then
		...
		# map to 11axa to generate hostapd config if hwmode is not configured in wireless
		[ "$hwmode" == "any" ] && {
			hwmode="11axa"
			is_hwmode_any=1
			if [ -n "$mld" ]; then
		...
		# these are in the right orders (most generic at the end)
			*ng:HT20)
				...
				[ "$vht_11ng" != "0" ] && append hostapd_cfg "ieee80211ac=1" "$N"
				;;
			*ng:HT40-)
				nl_hwmode=g
				append ht_capab "[$htmode]"
			...
			*ac:HT20|*ac:VHT20)
				...
				;;
			*ac:HT40+|*ac:HT40-|*ac:HT40|*ac:VHT40)
				nl_hwmode=a
				append hostapd_cfg "ieee80211ac=1" "$N"
				append hostapd_cfg "ieee80211n=1" "$N"
				case "$channel" in
			...
			*axg:*) nl_hwmode=g
				...
				append hostapd_cfg "ieee80211ax=1" "$N"
				;;
			*beg:HT20|*beg:VHT20|*beg:HE20|*beg:EHT20)
				nl_hwmode=g
				append ht_capab "[$htmode]"
				[ "$shortgi" != "0" ] &&  append ht_capab "[SHORT-GI-20]"
				append hostapd_cfg "ieee80211n=1" "$N"
			...
			*beg:HT40+ | *beg:EHT40+)
				...
				append hostapd_cfg "ieee80211be=1" "$N"
				;;
			*beg:HT40|*beg:VHT40|*beg:HE40|*beg:EHT40)
				nl_hwmode=g
				case "$channel" in
					1|2|3|4|5|6|7) append ht_capab "[HT40+]";;
					8|9|10|11|12|13) append ht_capab "[HT40-]";;
			...
			*axa:HT20|*axa:VHT20|*axa:HE20)
				...
				;;
			*axa:HT40+|*axa:HT40-|*axa:HT40|*axa:VHT40|*axa:HE40)
				nl_hwmode=a
				append hostapd_cfg "ieee80211ac=1" "$N"
				append hostapd_cfg "ieee80211n=1" "$N"
				append hostapd_cfg "ieee80211ax=1" "$N"
			...
			*axa:HT80|*axa:VHT80|*axa:HE80)
				...
				append hostapd_cfg "he_oper_centr_freq_seg0_idx=$idx" "$N"
				;;
			*axa:HT160|*axa:VHT160|*axa:HE160)
				nl_hwmode=a
				append hostapd_cfg "ieee80211ac=1" "$N"
				append hostapd_cfg "ieee80211n=1" "$N"
				append hostapd_cfg "ieee80211ax=1" "$N"
			...
			*axa:*) nl_hwmode=a
				...
				append hostapd_cfg "ieee80211ac=1" "$N"
				append hostapd_cfg "ieee80211ax=1" "$N"
				[ "$band" -eq 3 ] && append hostapd_cfg "op_class=132" "$N"
				;;
			...
			*bea:HT40+|*bea:HT40-|*bea:HT40|*bea:VHT40|*bea:HE40|*bea:EHT40+|*bea:EHT40-|*bea:EHT40)
				...
				append hostapd_cfg "eht_oper_chwidth=0" "$N"
				append hostapd_cfg "eht_oper_centr_freq_seg0_idx=$idx" "$N"
				[ "$puncture_bitmap" != "0xffff" ] && append hostapd_cfg "punct_bitmap=$puncture_bitmap" "$N"
				;;
		...
		[ -n "$ht_capab" ] && {
			append hostapd_cfg "ht_capab=$ht_capab" "$N"
			ht_capab=
		}
		[ -n "$vht_capab" ] && {
			append hostapd_cfg "vht_capab=$vht_capab" "$N"
			vht_capab=
		}
		...
		[ $is_hwmode_any -eq 1 ] && hwmode="any"

		append hostapd_cfg "hw_mode=$hwmode" "$N"
		append hostapd_cfg "wmm_enabled=1" "$N"

		[ -n "$bintval" ] && append hostapd_cfg "beacon_int=$bintval" "$N"
		# limit 5G/6G acs to own band channels.
		if [ -z "$acs_freq_list" ] && [ $hwmode = "a" ]; then
			case "$band" in
				3) acs_freq_list="5955-7115";;
				*)
					if [ "$has_6GHz_link" -eq 1 ]; then
						acs_freq_list="5180-5400"
					else
						acs_freq_list="5180-5825"
					fi
				;;
			esac
		fi
		...
		[ -n "$country" ] && {
			append hostapd_cfg "country_code=$country" "$N"
			append hostapd_cfg "ieee80211d=1" "$N"
			[ "$hwmode" = "a" -a "$doth" -gt 0 ] && append hostapd_cfg "ieee80211h=1" "$N"
		}
	...
	if [ -n "${mld}" ] && [ "$num_ready_link" -gt 0 ]; then
		rm -f ${wlan_module_path}/hostapd-$ifname-$num_ready_link.conf
		cat > ${wlan_module_path}/hostapd-$ifname-$num_ready_link.conf <<EOF
driver=$driver
interface=$ifname
...
EOF
		[ -z "${no_nconfig}" ] &&
			echo ${hwmode:+hw_mode=${hwmode#11}} >> ${wlan_module_path}/hostapd-$ifname-$num_ready_link.conf
	else
		rm -f ${wlan_module_path}/hostapd-$ifname.conf
		cat > ${wlan_module_path}/hostapd-$ifname.conf <<EOF
driver=$driver
interface=$ifname
${channel:+channel=$channel}
$hostapd_cfg
EOF
		[ -z "${no_nconfig}" ] &&
			echo ${hwmode:+hw_mode=${hwmode#11}} >> ${wlan_module_path}/hostapd-$ifname.conf
	fi
	...
	# can configure this option from wireless file
	config_get force_hostapd_attach "$device" force_hostapd_attach
	if [ "$force_hostapd_attach" -eq 1 ]; then
		if [ "$hwmode" != "*g*" ] && [ "$band" -ne 3 ] && [ "$htmode" == "HT20" ] && [ "$channel" -ne 0 ]; then
			# Delay to avoid potential DFS HT20 fixed-channel AP to start firstly
			sleep 1
		fi
		if [ $ezmesh_enable -ne 0 ] || [ -n "$mld" ]; then
			if [ -r ${wlan_module_path}/hostapd-global.pid ]; then
				if [ -n "$mld" ] && [ "$num_ready_link" -gt 0 ]; then
					wpa_cli -g ${wlan_module_path}/${hostapd_global_ctrl_interface} raw ADD bss_config=$ifname:${wlan_module_path}/hostapd-$ifname-$num_ready_link.conf &> /dev/null
				else
					wpa_cli -g ${wlan_module_path}/${hostapd_global_ctrl_interface} raw ADD bss_config=$ifname:${wlan_module_path}/hostapd-$ifname.conf &> /dev/null
					while [ $yocto_build -eq 1 -a -z "$(iw dev $ifname info | grep ssid)" ]; do
						sleep 0.5
					done
				fi
				if [ -n "$mld" ]; then
					num_ready_link=$((num_ready_link+1))
					uci set wireless.${mld}.num_ready_link=$num_ready_link
				fi
			else
				qlog_cmd "hostapd-global not started"
			fi
		else
			[ -f "${wlan_module_path}/hostapd-$ifname.lock" ] &&
				rm ${wlan_module_path}/hostapd-$ifname.lock
			if [  -f /etc/init.d/hostapd-daemon.init ]; then
				/etc/init.d/hostapd-daemon.init restart $ifname > /dev/kmsg 2>&1
			else
				echo "Not starting hostapd. Init files missing" > /dev/kmsg
			fi
		fi

		if [ "$dpp" -eq 1 ]; then
			hostapd_dpp_config "$vif" "$device"
		fi
	fi
}

hostapd_teardown_vif() {
	local ifname="$1"

	if [ -r ${wlan_module_path}/hostapd-global.pid ]; then
		echo "hostapd_teardown_vif(${ifname})" > /dev/kmsg
		wpa_cli -g ${wlan_module_path}/${hostapd_global_ctrl_interface} raw REMOVE ${ifname} &> /dev/null
	fi

	if [ ! -f "${wlan_module_path}/hostapd-${ifname}.pid" ]; then
		return
	fi

	if [ ! -f /etc/init.d/hostapd-daemon.init ]; then
		echo "hostapd_teardown_vif(${ifname}) fail: Init files missing" > /dev/kmsg
		return
	fi

	echo "hostapd_teardown_vif(${ifname})" > /dev/kmsg
	/etc/init.d/hostapd-daemon.init stop ${ifname} > /dev/kmsg 2>&1
}
...
run_hostapd_cli() {
	...
	ctrl_path=$2
	arguments=$3
	qlog_cmd "run_hostapd_cli, ifname=$1, ctrl_path=$2 arguments=$3"
	hostapd_cli -i $ifname $arguments
}
...
hostapd_dpp_config() {
	...
	pid=${wlan_module_path}/hostapd_cli-$ifname.pid
	run_hostapd_cli $ifname "${wlan_module_path}/hostapd-$device" "-P $pid -a /lib/wifi/dpp-hostapd-update-uci -B"

	run_hostapd_cli $ifname "${wlan_module_path}/hostapd-$device" "DPP_CONTROLLER_STOP"

	hostapd_cli -i $ifname -p ${wlan_module_path}/hostapd-$device dpp_bootstrap_remove \*
	hostapd_cli -i $ifname -p ${wlan_module_path}/hostapd-$device dpp_pkex_remove \*
	hostapd_cli -i $ifname -p ${wlan_module_path}/hostapd-$device dpp_configurator_remove \*
	if [ "$map" -ge 3 -a $band -ne 3 ]; then
		run_hostapd_cli $ifname  "${wlan_module_path}/hostapd-$device" "DPP_BOOTSTRAP_GEN $dpp_type $dpp_curve $dpp_key"
	else
		run_hostapd_cli $ifname  "${wlan_module_path}/hostapd-$device" "DPP_BOOTSTRAP_GEN $dpp_type $dpp_curve  $channel mac=$(cat /sys/class/net/$ifname/address | sed 's/://g') $dpp_key"
	fi

	if [ "$dpp_mud_url" ]; then
		run_hostapd_cli $ifname "${wlan_module_path}/hostapd-$device" "SET dpp_mud_url $dpp_mud_url"
	fi
	...
	if [ -z "$dpp_controller" ] && [ -z "$map" ] && [ "$map" -eq 0 ]; then
		if [ "$dpp_auth_role" == "initiator" ]; then
			...
			if [ "${type}" == "qrcode" ]
			then
				run_hostapd_cli $ifname  "${wlan_module_path}/hostapd-$device" "dpp_auth_init peer=1 role=enrollee $neg_freq $tcp_addr"
			else
				run_hostapd_cli $ifname  "${wlan_module_path}/hostapd-$device" "dpp_pkex_add own=1 $pkex_identifier role=enrollee $pkex_code $tcp_addr"
			fi
		else
			#Responder configuration
			if [ "dpp_over_tcp" -ne 0 ]; then
				run_hostapd_cli $ifname "${wlan_module_path}/hostapd-$device" "DPP_CONTROLLER_START qr=single"
			fi

			if [ "${type}" == "qrcode" ]; then
				run_hostapd_cli $ifname  "${wlan_module_path}/hostapd-$device" "dpp_listen $dpp_freq role=enrollee"
			else
				run_hostapd_cli $ifname  "${wlan_module_path}/hostapd-$device" "dpp_pkex_add own=1 $pkex_identifier role=enrollee $pkex_code"
			fi

			if [ $dpp_chirp -eq 1 ]; then
				run_hostapd_cli $ifname "${wlan_module_path}/hostapd-$device" "dpp_chirp own=1 iter=10 listen=$dpp_freq"
			fi
